...
function PaymentMethodInt() {
  ...
  const subscriptionId = createPaymentIntentMutation.data?.data?.subscriptionId || null;
  ...
  if (paymentClientSecret === null) {
    ...
    router.push(`/interpreter/payment/payment-confirmation/?subscriptionId=${subscriptionId}`);
  }
                  ...
                  {paymentClientSecret ? (
                    <Elements stripe={stripePromise}>
                      <PaymentProcessor
                        ...
                        onSuccess={() =>
                          router.push(
                            `/interpreter/payment/payment-confirmation/?subscriptionId=${subscriptionId}`
                          )
                        }
                        onError={error => {
                    ...
                    <Box className='wrapPlanInfo'>
                      ...
                      <List disablePadding>
                        <ListItem disablePadding>
                          ...
                          {/*  I know this will cause issue regarding plan price ui. still manager and backend dev Shubhrojyoti told to do this */}
                          <Typography variant='caption'>
                            $
                            {selectedPlan?.free_trial_in_days &&
                            selectedPlan?.free_trial_in_days > 0
                              ? 0
                              : selectedPlan?.price}
                          </Typography>
                        </ListItem>
                    ...
                    <Box className='wrapPlanInfo noBorder'>
                      ...
                      <List disablePadding>
                        <ListItem disablePadding>
                          ...
                          <Typography variant='caption'>{interpreterData?.full_name}</Typography>
                        </ListItem>
                        <ListItem disablePadding>
                          <Typography variant='body1'>Email Address</Typography>
                          <Typography variant='caption'>{interpreterData?.email}</Typography>
                        </ListItem>
                        <ListItem disablePadding>
                          <Typography variant='body1'>Phone Number</Typography>
                          <Typography variant='caption'>{interpreterData?.phone}</Typography>
                        </ListItem>
                      </List>
                    </Box>
                  </Box>
...
export default PaymentMethodInt;
