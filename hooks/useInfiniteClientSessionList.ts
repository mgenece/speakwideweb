import { getClientSessions } from '@/api/functions/session.api';
import { queryKeys } from '@/config/constants';
import { useInfiniteQuery } from '@tanstack/react-query';
import { useMemo } from 'react';

interface UseInfiniteClientSessionListProps {
...
export const useInfiniteClientSessionList = ({
  listType,
  itemsPerPage = 10,
  search,
  sortField,
  sortOrder,
}: UseInfiniteClientSessionListProps) => {
  const query = useInfiniteQuery({
    queryKey: queryKeys.clientSessionList(`${listType}-${search}-${sortField}-${sortOrder}`),
    queryFn: ({ pageParam = 1 }) =>
      getClientSessions({
        list_type: listType,
        page: pageParam,
